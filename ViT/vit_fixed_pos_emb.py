...
class PatchEmbedding(nn.Module):
    def __init__(self, image_size, patch_size, in_channels, embed_dim):
        ...
        self.num_patches = ((image_size + patch_size - 1) // patch_size) ** 2

        self.projection = nn.Conv2d(in_channels, embed_dim, kernel_size=patch_size, stride=patch_size)
        
        # 고정된 사인, 코사인 포지션 임베딩을 초기화
        self.positional_embedding = self.create_fixed_position_embedding(self.num_patches, embed_dim)

    def create_fixed_position_embedding(self, num_positions, embed_dim):
        with torch.no_grad():
            position_embedding = torch.zeros(num_positions, embed_dim).to()
            position = torch.arange(0, num_positions, dtype=torch.float).unsqueeze(1)
            div_term = torch.exp(torch.arange(0, embed_dim, 2).float() * (-math.log(10000.0) / embed_dim))
            
            position_embedding[:, 0::2] = torch.sin(position * div_term)  # 짝수 인덱스
            position_embedding[:, 1::2] = torch.cos(position * div_term)  # 홀수 인덱스
            
            # (1, num_positions, embed_dim)으로 차원 변경
            position_embedding = position_embedding.unsqueeze(0).to("cuda")
...
def main():
    ...
    parer.add_argument('--log_dir', type=str, default='./log_fix')
    parer.add_argument('--name', type=str, default='vit_fixpos_cifar100')
