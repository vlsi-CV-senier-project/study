...
class FCMAE(nn.Module):
    ...
    def __init__(
                ...
                dims=[96, 192, 384, 768],
                decoder_depth=1,
                decoder_embed_dim=768,  # dims[-1]과 동일하게 설정
                patch_size=4,
                mask_ratio=0.6,
                norm_pix_loss=False):
        super().__init__()
        ...
        # encoder
        self.encoder = ConvNeXtV2(in_chans=in_chans, depths=depths, dims=dims)
        # decoder
        self.proj = nn.Conv2d(
            in_channels=dims[-1], 
        ...
        decoder = [Block(dim=decoder_embed_dim, drop_path=0.) for i in range(decoder_depth)]
    ...
    def forward_decoder(self, x, mask):
        ...
        # append mask token
        n, c, h, w = x.shape
        mask = mask.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, h, w) # (N, 1, H, W)
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        # Reshape mask to match the dimensions of loss
        N, L = loss.shape
        mask = mask.reshape(N, -1)[:, :L]
    ...
    def forward(self, imgs, labels=None, mask_ratio=0.6):
        ...
        # Reshape the mask to match the patch size
        mask = mask.reshape(-1, imgs.shape[2] // self.patch_size, imgs.shape[3] // self.patch_size)
        mask = mask.flatten(1)  # [N, L]
