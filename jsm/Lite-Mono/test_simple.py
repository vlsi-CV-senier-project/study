...
def test_simple(args):
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            # Extract depth at the given coordinates
            depth_map = depth.cpu().numpy().squeeze()

            # Convert float coordinates to integer pixel values
            x_pixel = int(args.x * original_width)
            y_pixel = int(args.y * original_height)

            depth_value = get_depth_at_pixel(depth_map, x_pixel, y_pixel)
            print(f"Depth at ({args.x}, {args.y}) [pixel coordinates: ({x_pixel}, {y_pixel})]: {depth_value:.2f} meters")
